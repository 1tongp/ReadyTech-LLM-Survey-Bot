 ...
 * 3) Open survey (Drawer) & save updated guideline
 * 4) Generate share link (in Drawer) & capture token
...
test('Admin → link → take & chat → delete (confirm modal)', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /survey admin/i })).toBeVisible()
  ...
  await expect(page.getByText(/share link activated/i)).toBeVisible()
  await page.screenshot({ path: 'e2e-screens/02-admin-after-create.png', fullPage: true })

  // 3) Open survey details (Drawer) & save updated guideline for Q1
  const row = page
    .locator('tr')
    .filter({ hasText: 'E2E Admin Survey - Playwright Automation Testing' })
    .first()
  ...
  await expect(
    drawer.getByText(/Survey #\d+:\s*E2E Admin Survey - Playwright Automation Testing/i),
  ).toBeVisible()

  // Input Guideline for Q1
  const q1GuidelineTextarea = drawer.locator('textarea').first()
  await q1GuidelineTextarea.fill('must say good; add at least one reason')
  await drawer
    .getByRole('button', { name: /^save guideline$/i })
    .first()
    .click()
  await expect(page.getByText(/guideline saved/i)).toBeVisible()
  await page.screenshot({
    path: 'e2e-screens/03-admin-detail-after-guideline-save.png',
    fullPage: true,
  })

  // 4) Generate shareable link & capture token
  await drawer.getByRole('button', { name: /generate shareable link/i }).click()
  const modal = page.locator('.ant-modal-content').first()
  await expect(modal.getByText(/^Token:/)).toBeVisible()
  const token = (await modal.locator('code').first().innerText()).trim()
  ...
  await expect(
    page.getByText(/E2E Admin Survey - Playwright Automation Testing/i).first(),
  ).toBeVisible()
  ...
  await page
    ...
    .fill('We improved p95 latency and reduced 500s; outcomes were measurable.')
  await page.getByRole('button', { name: /^save$/i }).click()
  await expect(page.getByText(/saved/i)).toBeVisible()
  ...
  await page.getByRole('button', { name: /^next$/i }).click()
  await page
    .getByPlaceholder(/Your answer to Q2/i)
    .fill('impact: latency down, errors down, cost down')
  await page.getByRole('button', { name: /^save$/i }).click()
  await expect(page.getByText(/saved\./i)).toBeVisible()
  ...
  const confirmModal = page
    ...
    .filter({ hasText: /Delete survey/i })
    .first()
  ...
  await expect(
    page.locator('tr', { hasText: 'E2E Admin Survey - Playwright Automation Testing' }),
  ).toHaveCount(0)
