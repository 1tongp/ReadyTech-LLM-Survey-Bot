    ...
    ;(async () => {
      try {
        ...
        setChat([
          {
            role: 'bot',
            text: `Welcome! I’ll guide you through ${data.questions.length} question(s).`,
          },
        ...
        print(e)
        setChat([
          { role: 'bot', text: '⚠️ Invalid or inactive link. Please contact the survey owner.' },
        ])
      }
    })()
  }, [token])
    ...
    >
      <div style={{ padding: '12px 12px 0' }}>
        <Typography.Paragraph type="secondary" style={{ marginBottom: 8 }}>
          Chat mode:{' '}
          {readOnly
            ? 'This survey is read-only.'
            : 'answer below, or use actions. I’ll reply with status and scoring.'}
      ...
      <div style={{ borderTop: '1px solid #f0f0f0', padding: 12 }}>
        ...
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          />
          <Space wrap>
            <Button onClick={goPrev} disabled={!currentQ || idx === 0}>
              Previous
            </Button>
            <Button
              onClick={onSend}
              type="primary"
              loading={sending}
              disabled={!currentQ || !draft?.trim() || readOnly}
            >
              Save
            </Button>
            <Button onClick={goNext} disabled={!currentQ || idx === questions.length - 1}>
              Next
            </Button>
            <Button onClick={() => onFlagToggle(!flagged)} disabled={readOnly}>
              {flagged ? 'Unflag' : 'Flag'}
            </Button>
            <Button type="primary" onClick={onSubmitAll} disabled={readOnly || !answers.length}>
              Submit
            </Button>
          </Space>
        </Space>
